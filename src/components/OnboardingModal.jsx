        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <input name="company" placeholder="Optional" value={form.company} onChange={handleChange} style={inputSt} />
            </div>
          </div>

          <div>
            <label style={labelSt}>Location</label>
            <input name="location" placeholder="e.g. Hyderabad, India" value={form.location} onChange={handleChange} style={inputSt} />
          </div>

          <div>
            <label style={labelSt}>Short Bio</label>
            <textarea name="bio" placeholder="What are you building?" value={form.bio} onChange={handleChange} rows={3} style={{ ...inputSt, resize: 'vertical' }} />
          </div>
          ...
          <button type="submit" disabled={loading} className="btn btn-primary" style={{ width: '100%', justifyContent: 'center', marginTop: 8, padding: '14px', fontSize: '1rem', opacity: loading ? 0.7 : 1 }}>
            {loading ? 'Saving Profile...' : 'Join Community'} <ArrowRight size={18} />
          </button>
        </form>
      </div>
    </div>
