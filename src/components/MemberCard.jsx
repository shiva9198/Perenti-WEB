...
export function MemberCardHorizontal({ member }) {
  return (
    <Link to={`/profile/${member.id}`} className="member-card-horizontal" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', width: '100%', overflow: 'hidden' }}>
      <Avatar src={member.avatar} name={member.name} size="md" style={{ borderRadius: 12, flexShrink: 0 }} />
      <div className="member-info" style={{ flex: 1, minWidth: 0, paddingLeft: 'var(--space-3)', overflow: 'hidden' }}>
      ...
      {member.tags && member.tags[0] && (
        <div style={{ flexShrink: 0, paddingLeft: 8, paddingRight: 4, maxWidth: 90 }}>
          <span style={{
            display: 'block',
            maxWidth: '100%',
            overflow: 'hidden',
