export function parseMeetupTimes(dateStr, timeStr) {
  try {
    ...
    let endTimeStr = parts.length > 1 ? parts[1].replace(/\(.*\)/g, '').trim() : startTimeStr;

    const startParsed = Date.parse(`${cleanDate} ${startTimeStr}`);
    const endParsed = Date.parse(`${cleanDate} ${endTimeStr}`);

    const start = isNaN(startParsed) ? new Date(Date.now() - 1000) : new Date(startParsed);
    let end = isNaN(endParsed) ? new Date(start.getTime() + 2 * 60 * 60 * 1000) : new Date(endParsed);
